...
def _generate_result_pdf(
    *,
    ...
    styles.add(ParagraphStyle(name="SectionHeading", parent=styles["Heading2"], fontName="Helvetica-Bold", fontSize=12.5, textColor=colors.HexColor("#1d4ed8"), leading=16, spaceBefore=10, spaceAfter=6))
    styles.add(ParagraphStyle(name="BodyTextSmall", parent=styles["BodyText"], fontName="Helvetica", fontSize=9.2, leading=12, textColor=colors.HexColor("#334155")))
    styles.add(ParagraphStyle(name="BodyTextMuted", parent=styles["BodyText"], fontName="Helvetica", fontSize=8.6, leading=11, textColor=colors.HexColor("#64748b")))
    ...
    def _table_from_items(title: str, items: list[tuple[str, str]]) -> None:
        ...
        table = Table(rows, colWidths=[50 * mm, 115 * mm], repeatRows=1)
    ...
    def _draw_header_footer(canvas, _doc):
        ...
        canvas.drawString(16 * mm, A4[1] - 16 * mm, "Smart Lender")
        canvas.setFont("Helvetica", 9)
        canvas.drawRightString(A4[0] - 16 * mm, A4[1] - 16 * mm, "AI Powered Loan Approval System")
        canvas.setFillColor(colors.HexColor("#64748b"))
        canvas.setFont("Helvetica", 8)
        canvas.drawString(16 * mm, 10 * mm, "Generated by Smart Lender")
        canvas.drawRightString(A4[0] - 16 * mm, 10 * mm, f"Downloaded on: {prediction_time.strftime('%d %b %Y, %I:%M %p')}")
        canvas.restoreState()
